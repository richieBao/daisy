...
class DevSMA(Optimizer):
    ...
    def __init__(self, epoch: int = 10000, pop_size: int = 100, p_t: float = 0.03, **kwargs: object) -> None:
        ...
        # if self.h5py_fn is None:
        #     self.h5py_fn='processData/pData_SMA.hdf5'
        
        if self.pickle_fn is None:
            self.pickle_fn='processData/pData.pkl'
        
            
        # self.h5py_processData=h5py.File(self.h5py_fn,"w")
    ...
    def evolve(self, epoch):
        ...
        """        
        self.processData_epoch={}
        # print(self.pop[-1].solution)
        
        # plus eps to avoid denominator zero 
        ss = self.g_best.target.fitness - self.pop[-1].target.fitness + self.EPSILON
        # ss = self.g_best.target.fitness - self.pop_sorted[-1].target.fitness + self.EPSILON
        
        
        #print(f"------epoch={epoch};selfEpoch={self.epoch};ss={ss};pop={self.pop[0]};g_best={self.g_best};weights={self.weights}")
        # calculate the fitness weight of each slime mold
        processData_epoch_weight={"bF":self.g_best.target.fitness,"wF":self.pop[-1].target.fitness,"Si":[]}
        for idx in range(0, self.pop_size):
            # print(f"...{self.generator.uniform(0, 1, self.problem.n_dims)}")
            # Eq.(2.5)
            ...
            processData_epoch_weight["Si"].append(self.pop[idx].target.fitness)
                                    
        self.processData_epoch['weightEpoch']=processData_epoch_weight                            
        ...
        for idx in range(0, self.pop_size):
            ...
            if self.generator.uniform() < self.p_t:  # Eq.(2.7)
                ...
                #print(f"idab={id_a},{id_b},ida_solution={self.pop[id_a].solution}")
