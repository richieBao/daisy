...
class Optimizer:
    ...
    def solve(self, problem: Union[Dict, Problem] = None, mode: str = 'single', n_workers: int = None,
        ...
        for epoch in range(1, self.epoch + 1):
            #print(f"###g_best={self.g_best};epoch-selfEpoch={epoch}-{self.epoch}")
            ...
            pop_temp, self.g_best, self.indices= self.update_global_best_agent(self.pop)
            # self.pop_sorted=pop_temp
            if self.sort_flag: self.pop = pop_temp
    ...
    def update_global_best_agent(self, pop: List[Agent], save: bool = True) -> Union[List, Tuple]:
        ...
        """
        if self.problem.return_index:
            sorted_pop, indices= self.get_sorted_population(pop, self.problem.minmax,self.problem.return_index)
        else:
        ...
        if save:
            ...
            return sorted_pop, better,indices
        else:
            ## Handle current best
            ...
            return sorted_pop, global_better,indices
